...
<section id="s95" style="background:#f7f9fc;">
  ...
  <p>
    ...
    (<code>scripts/backtest_random_benchmark.py</code>)。
  </p>
  <table>
  ...
  <p>
    ...
    正面比較した (5y、<code>scripts/backtest_vix_compare.py</code>)。
  </p>
  <table>
...
<section id="s5">
  ...
  (5y × 40 銘柄、持続ホモロジーも再実行。<code>scripts/independence_robustness.py</code>)。</p>
  <table>
...
<section id="s86">
  ...
  再現: <code>scripts/loo_multi_shock.py</code>、<code>data/loo_multi_shock.json</code>。</p>
  <table>
...
<section id="s11">
  ...
  <table>
    ...
    <tr><td><strong>銘柄構成バリエーション総当たり</strong></td>
        ...
        <td>20-30 銘柄で頑健。4 ショック LOO で全共通の普遍核は NGAS のみ (4/4)、COPPER/CHINA50 が準ずる (各 3/4)、周辺はイベント依存の二層構造 (<code>scripts/loo_multi_shock.py</code>)</td></tr>
