...
<section id="s95" style="background:#f7f9fc;">
  ...
  <h3 style="margin-top:28px;">9.5.5 <span style="color:var(--green); font-size:0.65em;">● 確実</span> 「ただ半分現金にしてるだけ」じゃない証明 — ランダム現金化との対決</h3>
  <p>
    一番鋭い疑問: <strong>「この戦略、全期間の約半分 (47%) を現金化してる。MaxDD が浅いのは
    e_div が暴落を当ててるからじゃなく、ただ株を半分しか持ってないからでは?」</strong><br>
    これに答えるため、<strong>e_div を無視して、同じ現金化率・同じ取引回数でランダムに現金化した戦略を
    1000 通り</strong>シミュレーションし、e_div がその分布のどこに位置するかを見た
    (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/backtest_random_benchmark.py"><code>scripts/backtest_random_benchmark.py</code></a>)。
  </p>
  <table>
    <tr><th>期間</th><th>e_div の MaxDD</th><th>ランダム平均 MaxDD</th>
        <th>e_div の Sharpe</th><th>ランダム平均 Sharpe</th>
        <th>ランダムが e_div に勝つ確率</th></tr>
    <tr><td>5y</td><td class="good">-15.6%</td><td>-21.6%</td>
        <td class="good">+0.85</td><td>+0.41</td>
        <td>MaxDD 15% / Sharpe 8%</td></tr>
    <tr><td><strong>20y</strong> (2008/2020 含む)</td>
        <td class="good"><strong>-20.4%</strong></td><td>-42.0%</td>
        <td class="good"><strong>+0.76</strong></td><td>+0.27</td>
        <td><strong>MaxDD 0% / Sharpe 0.1%</strong></td></tr>
  </table>
  <div class="callout found">
  ...
  <p>
    次の疑問: <strong>「結局ボラが高い時に避けてるだけなら、恐怖指数 VIX で同じことができるのでは?」</strong><br>
    e_div と<strong>同じ現金化率になるよう VIX の閾値を調整</strong>して、「VIX が高い日に避ける」戦略と
    正面比較した (5y、<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/backtest_vix_compare.py"><code>scripts/backtest_vix_compare.py</code></a>)。
  </p>
  <table>
    <tr><th>戦略 (同じ ~48% 現金化)</th><th>Sharpe</th><th>MaxDD</th><th>リターン</th></tr>
    <tr><td><strong>e_div で避ける</strong></td>
        <td class="good"><strong>+0.85</strong></td><td>-15.6%</td><td class="good"><strong>+55.3%</strong></td></tr>
    <tr><td>VIX で避ける</td><td class="bad">+0.06</td><td>-15.7%</td><td class="bad">+2.1%</td></tr>
    <tr><td>ランダムに避ける</td><td>+0.40</td><td>-21.8%</td><td>—</td></tr>
  </table>
  <div class="callout found">
    <h4>結論: VIX では代用できない (符号構造ならではの情報がある)</h4>
    <p><strong>VIX で避けると、ランダムに避けるより悪い (+0.06 &lt; +0.40)</strong>。
    理由: VIX が高い = <strong>既に暴落が起きた後</strong>。後追いで底値圏で現金化し、回復を取りこぼす。
    一方 e_div は +0.85 で圧勝。<br>
  ...
  <p>
    ...
    (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/backtest_oos_random_vix.py"><code>scripts/backtest_oos_random_vix.py</code></a>、20y で 16 fold)。閾値・z-score・VIX 閾値すべて
...
<section id="s105" style="background:#f0f7ff;">
  ...
  実装: <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/eventstudy_8y_oos.py"><code>scripts/eventstudy_8y_oos.py</code></a>, <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/data/events_8y.json"><code>data/events_8y.json</code></a>,
...
<section class="hero">
  <div class="hero-inner">
    ...
    <p class="meta">Market Graph Research · 2026</p>
...
<nav class="toc">
  ...
  <div class="toc-inner">
    ...
    <a href="#s5">5. 独立性 <span style="color:var(--green)">●</span></a>
    <a href="#s6">6. ショック反応 <span style="color:var(--orange)">◐</span></a>
    <a href="#s7">7. 符号反転 <span style="color:var(--orange)">◐</span></a>
    <a href="#s8">8. e_div 判別器 <span style="color:var(--orange)">◐</span></a>
    <a href="#s85">8.5 指標細分化</a>
    <a href="#s86">8.6 源泉解剖 <span style="color:var(--green)">●</span></a>
    <a href="#s9">9. 圏論的整理</a>
...
<section id="s1">
  ...
  <div class="callout intuition">
    ...
    <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/robustness_subsample.py"><code>scripts/robustness_subsample.py</code></a> 参照)。</p>
  </div>
</section>
...
<section id="s2">
  ...
  <p style="font-size:12px; color:var(--sub);">
    ...
    実証で window=20/30/60/90 を比較 (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/robustness_window.py"><code>scripts/robustness_window.py</code></a>)。
...
<section id="s5">
  ...
  <h2><span style="color:var(--green); font-size:0.7em; vertical-align:middle;">● 確実な発見</span><br>発見 1: 2 つの指標は独立に動く</h2>
  <p class="lede">理論的に「違う情報を見ている」と言ったが、実データでも本当に独立に動くかを確認。
  <strong>全 10 設定 (window/threshold/市場) で |r|&lt;0.5 が再現する、最も確実な発見</strong>。</p>
  ...
  <div class="callout found">
    ...
    データ期間を伸ばすと相関は<strong>単調に上昇</strong>する (5y/10y/15y/20y 米国 = 0.16/0.19/0.26/<strong>0.40</strong>)。
    <strong>短期 (5 年) ではほぼ独立だが、20 年フルでは r=0.40 に達し独立とは言えなくなる</strong>。
    理由: 長期は危機 (2008/2020 等) を多く含み、危機時は L¹ も不整合サイクル数も同時に跳ねて連動するため。
    市場別は新興国 0.17・中国 0.41。<strong>「独立」は平常時・短期の近似</strong>であって、厳密ではない。</p>
  </div>

  <h3>設定を変えても独立か — 頑健性チェック</h3>
  <p>「window=30・threshold=0.3 だから偶然 0.16 になっただけでは?」に答えるため、
  window・threshold・市場を変えて L¹ と不整合サイクル数の相関 r を再計算した
  (5y × 40 銘柄、持続ホモロジーも再実行。<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/independence_robustness.py"><code>scripts/independence_robustness.py</code></a>)。</p>
  <table>
    <tr><th>変える軸</th><th>設定</th><th>相関 r</th></tr>
    <tr><td rowspan="4">window<br>(閾値 0.3 固定)</td><td>20 日</td><td>+0.23</td></tr>
    <tr><td>30 日</td><td>+0.16</td></tr>
    <tr><td>40 日</td><td>+0.06</td></tr>
    <tr><td>60 日</td><td>−0.06</td></tr>
    <tr><td rowspan="3">threshold<br>(窓 30 固定)</td><td>0.2</td><td>+0.22</td></tr>
    <tr><td>0.3</td><td>+0.16</td></tr>
    <tr><td>0.4</td><td>+0.05</td></tr>
    <tr><td rowspan="3">市場<br>(窓 30 / 閾値 0.3)</td><td>米国</td><td>+0.16</td></tr>
    <tr><td>新興国</td><td>+0.17</td></tr>
    <tr><td>中国</td><td>+0.41</td></tr>
  </table>
  <div class="callout found">
    <h4>結論: 短期の独立性は設定非依存。ただし期間依存で長期は崩れる</h4>
    <p>5 年内では全 10 設定で <strong>|r| &lt; 0.5</strong>、うち 9/10 は <strong>|r| &lt; 0.3</strong>
    (範囲 0.05〜0.41、平均 0.17)。window を 20〜60 日、threshold を 0.2〜0.4、市場を変えても
    <strong>短期の独立性は特定パラメータの「まぐれ」ではなく頑健</strong>。
    <strong>ただしデータ期間を 20 年に伸ばすと r=0.40 まで上がり、独立とは言えなくなる</strong> (上記)。
    独立は「短期・平常時の近似」であり、危機を多く含む長期では 2 指標は連動する。正直な限界として開示する。</p>
  </div>
</section>

<section id="s5b">
  <div class="section-num">SECTION 05B</div>
  <h2><span style="color:var(--green); font-size:0.7em; vertical-align:middle;">● 追加検証</span><br>なぜ 2 指標の「差分」か — 単独では効かない</h2>
  <p class="lede">e_div = z(n_unb) − z(L¹) という差分に本当に意味があるのか。
  単独指標 (n_unb だけ・L¹ だけ) と<strong>公平に比較</strong>
  (同じ閾値 0.8σ で株を避ける short 戦略、20 年 in-sample、取引コスト込み) して確かめた。</p>
  <table>
    <tr><th>指標</th><th>最大下落 (MaxDD)</th><th>Sharpe</th></tr>
    <tr><td><strong>e_div 差分</strong></td><td class="good"><strong>−20%</strong></td><td class="good"><strong>0.76</strong></td></tr>
    <tr><td>n_unb 単独 (矛盾)</td><td>−45%</td><td>0.47</td></tr>
    <tr><td>L¹ 単独 (穴)</td><td class="bad">−56%</td><td>0.22</td></tr>
    <tr><td>Buy &amp; Hold</td><td class="bad">−57%</td><td>0.48</td></tr>
  </table>
  <div class="callout found">
    <h4>単独では下落を防げない。差分にして初めて効く</h4>
    <p>L¹ 単独は −56% で Buy &amp; Hold (−57%) とほぼ同じ = 下落回避に<strong>無力</strong>。
    n_unb 単独も −45% どまり。ところが差分 e_div にすると <strong>−20% に激変</strong>する。
    これが「2 つの指標を組み合わせる意味」の直接証拠 — 単独ではダメで、差分にして初めて効く。</p>
  </div>

  <h3>機序 — なぜ差分が効くか (相対突出。ただし「時間差」ではない)</h3>
  <p>当初は「矛盾 n_unb が穴 L¹ より<em>先に</em>動く時間差を捉えている」と仮説を立てた。
  しかし 3 手法 (ラグ相関・危機イベントの立ち上がり時刻・Granger 因果) で厳密に検証した結果、
  <strong>時間差仮説は棄却</strong>された — n_unb は先行せず、むしろ弱く L¹ が先行するか、ほぼ同時だった。</p>
  <p>代わりに確認できたのは<strong>「相対突出」</strong>。e_div が高い日を調べると、
  矛盾 <strong>z(n_unb) ≈ +1.3</strong> が突出している一方、穴 <strong>z(L¹) ≈ −0.3</strong> はむしろ低い。
  つまり e_div は<strong>「穴を基準線にして、矛盾がそれを超えて突出した状態」</strong>を捉えている。
  L¹ が高い日は下落確率がむしろ全体より低く (下落と無関係) 単独では無力だが、
  それを<em>引く</em>ことで矛盾の「純粋な超過分」だけを取り出すのが、差分の効く理屈。</p>
  <div class="callout intuition">
    <h4>正直な限界 — 機序は完全には解けていない</h4>
    <p>ただし「e_div 高い → 翌 20 日に下落」という<strong>日次の予測力は弱い</strong>
    (下落確率 38% vs 全体 34%、差はわずか)。
    バックテストで MaxDD が大きく改善するのは、日次予測ではなく
    <strong>「危機の"期間"を識別して、その間ずっと現金でいる」経路依存効果</strong>による
    (最大下落は連続下落の累積なので、危機期間の継続的な現金化が効く)。
    「なぜ相対突出が下落局面と結びつくか」の完全な機序解明は今後の課題。</p>
  </div>
</section>

<section id="s6">
  <div class="section-num">SECTION 06</div>
  <h2><span style="color:var(--orange); font-size:0.7em; vertical-align:middle;">◐ 示唆的な観察 (有意性は限定的)</span><br>観察 2: ショックの種類で反応する指標が変わる (方向性)</h2>
  <p class="lede">市場ショック (戦争・関税・利上げ等) の前後で、2 つの指標は<strong>異なる感度</strong>で反応する傾向。
  <strong>ただし方向は見えるが、関税 11 件単独では集約 n_unb は有意でない (p=0.11)。
  有意なのは 4-cycle 成分 (+0.39, p=0.004) で、それも 2025-04 クラスタ駆動</strong> (下表 + Section 11 参照)。</p>
...
<section id="s7">
  ...
  <h2><span style="color:var(--orange); font-size:0.7em; vertical-align:middle;">◐ 示唆的な観察 (有意性は弱い)</span><br>観察 3: 関税前後の符号反転を直接検証 — 個別では起こるが、一般化は弱い</h2>
  <p class="lede">「関税ショックで銘柄ペアの符号が反転する」という仮説を flip_rate で直接検証。
  <strong>2025-04 では主要ペアが実際に反転したが、関税ショック一般 (11 件) では統計的有意性に届かなかった</strong>。
  正直な結果を示す。</p>

  <h3>仮説 (検証対象)</h3>
  <p>関税が発令されると貿易相手国との経済的位置関係が変わる →
  「これまで一緒に動いていた銘柄ペア」が「逆に動く」ようになる →
  <strong>符号反転</strong>が起こり不整合サイクル数が上がる、という仮説。
  <strong>これを直接検証した結果は「方向は正だが有意性は弱い」</strong> (下記)。</p>
  ...
  <div class="callout found">
    ...
    <p class="small">検証: <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/eventstudy_flip_rate_verify.py"><code>scripts/eventstudy_flip_rate_verify.py</code></a> →
...
<section id="s8">
  ...
  <h2><span style="color:var(--orange); font-size:0.7em; vertical-align:middle;">◐ 示唆段階 (2025-04 クラスタ依存)</span><br>観察 4: 2 指標の<strong>乖離</strong> e_div でショックタイプを区別できそう</h2>
  <p class="lede">「政策ショック」と「単なる市場ボラ」を区別する指標の<strong>候補</strong>。
  <strong>2025-04 Liberation Day では強く出た (Δσ=+1.57, p&lt;10⁻⁴) が、これは 5 件の連鎖クラスタ 1 件への依存が大きい</strong>。
  trade_policy 全体 (n=36) では +0.15 (p=0.09) と弱く、判別器としての一般性は VPS 運用データで検証中。</p>
  ...
  <p style="font-size:12px; color:var(--sub);">
    ...
    全ペア差分 55 通りで event study を試した (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/robustness_ediv_pair_scan.py"><code>scripts/robustness_ediv_pair_scan.py</code></a>)。
  ...
  <p style="font-size:12px; color:var(--sub);">
    ...
    (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/robustness_cluster_permutation.py"><code>scripts/robustness_cluster_permutation.py</code></a>)。
...
<section id="s86">
  ...
  <div class="callout warn" style="margin:10px 0;">
    <p style="margin:0;"><strong>⚠️ 重要な注意</strong>: 下表は <strong>2025-04 Liberation Day 単独</strong>の源泉ランキング。
    後述の<strong>解剖 8 で 4 ショックに拡張すると、この順位は大きく入れ替わる</strong> ——
    EUB10Y / USDJPY / VIX はこのイベント固有 (他ショックでは効かない) で、
    <strong>4 ショック全てで共通の普遍核は NGAS (天然ガス) のみ。COPPER・CHINA50 がそれに準ずる (各 3/4、円キャリーで脱落)</strong>。
    つまり「1 イベントの源泉ランキングを普遍と読んではいけない」。詳細は解剖 8。</p>
  </div>
  <table>
  ...
  <!-- ===== 解剖 8: 複数ショックで LOO 再現 ===== -->
  <h3 style="margin-top:36px; border-top:2px solid var(--accent); padding-top:24px;">
    解剖 8: 4 つの独立ショックで LOO を再現 — 震源は普遍か、イベント依存か</h3>
  <p>解剖 1 の源泉ランキングは <strong>2025-04 Liberation Day の 1 イベント</strong>だけだった。
  震源が本当に cross-asset なら、性質の違う他ショックでも同じ銘柄が源泉のはず。
  4 つの独立ショックで LOO を回し直した (z-score は <strong>expanding window</strong> で look-ahead 排除。
  このため Liberation Day baseline は +0.70σ と控えめに出る — 手法差)。
  再現: <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/loo_multi_shock.py"><code>scripts/loo_multi_shock.py</code></a>、<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/data/loo_multi_shock.json"><code>data/loo_multi_shock.json</code></a>。</p>
  <table>
    <tr><th>ショック</th><th>baseline Δe_div</th><th>源泉 TOP3 (抜くと弱くなる)</th></tr>
    <tr><td>2025-04 Liberation Day (関税)</td><td>+0.70</td><td>COPPER / EUB10Y / NGAS</td></tr>
    <tr><td>2024-08 円キャリー巻き戻し</td><td>+1.80</td><td>NGAS / META / US10Y</td></tr>
    <tr><td>2023-10 ハマス・イスラエル (戦争)</td><td>+0.95</td><td>COPPER / VIX / NGAS</td></tr>
    <tr><td>2022-02 ウクライナ侵攻 (戦争)</td><td>+0.49</td><td>NGAS / COPPER / CHINA50</td></tr>
  </table>
  <h4 style="margin-top:18px;">震源は「共通核 + イベント依存の周辺」の二層構造</h4>
  <table>
    <tr><th>銘柄</th><th>源泉になったショック数</th><th>解釈</th></tr>
    <tr><td><strong>NGAS</strong> (天然ガス)</td><td class="good">4 / 4</td><td><strong>唯一の普遍震源</strong></td></tr>
    <tr><td><strong>COPPER</strong> (銅)</td><td>3 / 4</td><td>準普遍</td></tr>
    <tr><td><strong>CHINA50</strong> (中国指数)</td><td>3 / 4</td><td>準普遍</td></tr>
    <tr><td>EUB10Y / USDJPY / VIX</td><td>各 1 / 4</td><td><strong>イベント依存</strong> (解剖 1 の Liberation 固有結果だった)</td></tr>
  </table>
  <p style="font-size:12px; color:var(--sub);">
    決定的検証: 核 3 銘柄 <strong>{NGAS, COPPER, CHINA50}</strong> を束で抜くと
    <strong>全 4 ショックで Δe_div 低下、2 ショックで符号反転</strong> (Liberation +0.70→-0.19、ウクライナ +0.49→-0.67)。
    「コモディティ・中国景気サイクル」のクロス連鎖が束で効いている証拠。<br>
    asset class 別では <strong>INDEX 抜きで崩壊するのは 1/4 (Liberation のみ)</strong> →
    解剖 2 の「INDEX が震源」も Liberation 固有だった。崩壊最多は COMMODITY/BOND (各 2/4)。
  </p>
  ...
  <div class="callout found" style="margin-top:24px;">
    ...
    e_div の構造的本体は米株ではなく <strong>cross-asset の連鎖</strong>にあった。
    さらに 4 ショックで再現すると (解剖 8)、全ショック共通の普遍核は
    <strong>NGAS のみ (4/4)。COPPER・CHINA50 が準ずる (各 3/4、商品・中国景気サイクル)</strong> で、
    株指数・債・VIX・円などの周辺はショックごとに入れ替わる
    <strong>「常駐する商品コア + イベント依存の周辺」の二層構造</strong>と判明した。</p>
...
<section id="s11">
  ...
  <h2>限界と今後</h2>
  <p class="lede">研究の正直な制約を全部出す。すべて実証検証済 (<a href="https://github.com/hajimedayo328/market-graph-presentation/tree/main/scripts"><code>scripts/audit_*.py</code> / <code>robustness_*.py</code></a>)。</p>
  ...
  <table>
    ...
    <tr><td><strong>銘柄構成バリエーション総当たり</strong></td>
        ...
        <td>20-30 銘柄で頑健。4 ショック LOO で全共通の普遍核は NGAS のみ (4/4)、COPPER/CHINA50 が準ずる (各 3/4)、周辺はイベント依存の二層構造 (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/loo_multi_shock.py"><code>scripts/loo_multi_shock.py</code></a>)</td></tr>
  ...
  <ol>
    ...
    <li><strong>z-score look-ahead 完全排除済み</strong>: バックテスト (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/backtest.py"><code>backtest.py</code></a>,
        <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/backtest_v2.py"><code>backtest_v2.py</code></a>, <a href="https://github.com/hajimedayo328/market-graph-presentation/tree/main/scripts"><code>backtest_walkforward*.py</code></a>) の z-score は全て
    ...
    <li><strong>因果性の検証範囲</strong>: 2 方向 Granger 因果性検定で
        ...
        (<a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/scripts/causal_granger.py"><code>scripts/causal_granger.py</code></a> / <a href="https://github.com/hajimedayo328/market-graph-presentation/blob/main/data/causal_granger_results.json"><code>data/causal_granger_results.json</code></a>)。
