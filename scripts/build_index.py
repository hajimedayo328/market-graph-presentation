...
<section id="s11">
  ...
  <ol>
    <li><strong>過去最適化バイアス</strong>: バックテストの閾値 0.8 / -0.5 は in-sample 選択。
        Walk-forward OOS で Sharpe +1.16 → +0.65 に低下しており、過去最適化を認容。
    ...
    <li><strong>z-score の weak look-ahead</strong>: 既存バックテストは全期間 mean/std で正規化 (実装簡便化)。
        過去のみ expanding window 計算と相関 0.99 で実質同一、主要日 (Liberation Day) でも 3σ 超は両方で出る。
    ...
    <li><strong>圏論的厳密性</strong>: 圏論は道具として使用、新定理や構造定理は得ていない。
        関手の極限 / pullback による手法非依存シグナル構築は実装途中。</li>
  ...
  <h3>11.3 今後の方向</h3>
  <ul>
    <li>20y データを使った関手の極限 / pullback 構築 → 手法非依存シグナル</li>
