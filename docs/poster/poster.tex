...
{\color{muted}\small 青＝正の相関・赤破線＝負の相関（40銘柄の相関ネットワーク）}
...
\[
L^1 = \sum_{k}(d_k - b_k)
\quad{\small (b_k\text{＝現れる基準},\ d_k\text{＝消える基準})}
\]
}
...
\block{\bt{3.3}2つは独立か}{%
指標1（穴）と指標2（矛盾）が、たがいにどれくらい似て動くかを相関で測った:
...
\begin{tabular}{lcl}
\toprule
{\bfseries データ期間} & {\bfseries 相関} & \\
\midrule
5年  & 0.15 & \color{accent}\textbf{ほぼ独立}\\
10年 & 0.19 & ほぼ独立\\
15年 & 0.26 & 弱い相関\\
...
\begin{itemize}
\setlength{\itemsep}{1mm}
\item \textbf{短期}：2指標はばらばらに動き\textbf{\color{accent}独立}（別々の情報を持つ）
\item \textbf{長期}：危機（2008・2020等）を多く含み、両指標が同時に跳ねて\textbf{\color{warn}相関が上がる}
\end{itemize}
}
